...
onMounted(async () => {
  try {
    ...
    scheduleCount.value = scRes.data.data?.total || 0
  } catch { /* 降级 */ }
})
...
const entries = [
  ...
  { title: '排班管理', desc: '管理教练排班与容量设置', count: scheduleCount, route: '/admin/schedules', color: 'bg-emerald-50 text-emerald-600' },
]
</script>
