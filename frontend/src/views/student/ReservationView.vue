...
import type { ReservationOption, Reservation } from '@/types/reservation'
...
async function handleQuery() {
  ...
  try {
    ...
    showToast(err?.response?.data?.message || '查询失败', 'error')
  } finally { loading.value = false }
}

async function handleBook() {
  if (!option.value?.mainCoachOption?.available) return
  const opt = option.value.mainCoachOption
  booking.value = true
  try {
    await createReservation(opt.scheduleId)
    showToast('预约成功')
    option.value = null
    await fetchMyReservations()
    handleQuery() // 刷新可预约状态
  } catch (err: any) {
    showToast(err?.response?.data?.message || '预约失败', 'error')
...
<template>
  <div class="max-w-4xl mx-auto">
    ...
    <div class="card p-6 mb-6">
      ...
      <div class="flex flex-wrap items-end gap-4">
        ...
        <button @click="handleQuery" :disabled="loading"
          class="px-5 py-2 bg-ink-600 text-white text-sm font-medium rounded-lg hover:bg-ink-700 disabled:opacity-50 transition-colors">
          {{ loading ? '查询中...' : '查询可预约状态' }}
        </button>
      </div>
    </div>
    ...
    <div v-if="option" class="card p-6 mb-6">
      ...
      <div class="flex items-start justify-between">
        <div>
          ...
          <div class="text-sm text-gray-500 mt-1">
            {{ option.mainCoachOption.scheduleDate }} {{ slotLabel[option.mainCoachOption.timeSlot] || option.mainCoachOption.timeSlot }}
          </div>
          ...
          <div class="flex items-center gap-3">
            <div class="w-24 bg-gray-100 rounded-full h-2">
              <div class="h-2 rounded-full transition-all"
                :class="option.mainCoachOption.remainCount > 0 ? 'bg-primary-500' : 'bg-red-400'"
                :style="{ width: ((option.mainCoachOption.currentStudents || 0) / (option.mainCoachOption.maxStudents || 1) * 100) + '%' }" />
            </div>
            <span class="text-sm font-medium" :class="option.mainCoachOption.remainCount > 0 ? 'text-gray-700' : 'text-red-500'">
              {{ option.mainCoachOption.currentStudents || 0 }}/{{ option.mainCoachOption.maxStudents }}
            </span>
      ...
      <button
        v-if="option.mainCoachOption.available"
        @click="handleBook" :disabled="booking"
        class="mt-4 w-full py-3 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-xl disabled:opacity-50 transition-colors"
      >
        {{ booking ? '预约中...' : '立即预约' }}
      </button>
    </div>
        ...
        <div v-for="r in myList" :key="r.id" class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
          <div>
            <div class="text-sm font-medium text-gray-800">
              {{ r.reservationDate }} {{ slotLabel[r.timeSlot] || r.timeSlot }}
            </div>
            <div class="text-xs text-gray-500 mt-0.5">
              教练：{{ r.actualCoachName }} <span v-if="r.plateNumber"> · {{ r.plateNumber }}</span>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', statusColor[r.status] || 'bg-gray-100']">
              {{ statusLabel[r.status] || r.status }}
            </span>
            <button v-if="r.status === 'SUCCESS'" @click="handleCancel(r)"
              class="text-xs text-red-500 hover:text-red-700 font-medium">取消</button>
          </div>
        </div>
      </div>
    </div>
